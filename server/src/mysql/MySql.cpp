...
my_ulonglong MySql::inster_user_info(char * databases, char *table,  USER_Table data){
    my_ulonglong affectedRows = 0; 
    char cmd_change_databases[40];
    sprintf(cmd_change_databases,"USE %s",databases);
    ...
    sprintf(cmd_str,"INSERT INTO %s (name,email, password,id_card,status,type,age)" 
                " VALUES ('%s', '%s', '%s', '%s', '%s', '%s', %d)",table,get<1>(data).c_str(),get<2>(data).c_str(),get<3>(data).c_str(),
...
my_ulonglong MySql::delete_user_info(char * databases, char *table, char* key, char *value){
    char cmd_change_databases[40];
    sprintf(cmd_change_databases,"USE %s",databases);
    runCommand(cmd_change_databases);
    char cmd_str[100];
    sprintf(cmd_str,"DELETE FROM %s WHERE %s = %s",table,key,value);
    // //删除数据
    const my_ulonglong affectedRows = runCommand(cmd_str);
    ////跟新主键
    //删除原主键
    sprintf(cmd_str,"alter table %s drop id",table);
    runCommand(cmd_str);
    //添加新主键
    sprintf(cmd_str,"alter table %s ADD id MEDIUMINT( 8 ) NOT NULL FIRST",table);
    runCommand(cmd_str);
    //设置自增
    sprintf(cmd_str,"alter table %s MODIFY COLUMN id MEDIUMINT( 8 ) NOT NULL AUTO_INCREMENT,ADD PRIMARY KEY(id)",table);
    runCommand(cmd_str);
    return affectedRows;
}

my_ulonglong MySql::update_user_info(char *databases, char *table, char* key_id, char* keyid_value, char* key, char *value){
    char cmd_change_databases[40];
    sprintf(cmd_change_databases,"USE %s",databases);
    runCommand(cmd_change_databases);
    char cmd_str[100];
    sprintf(cmd_str,"UPDATE %s  set %s = \"%s\" WHERE %s = \"%s\"",table,key,value,key_id,keyid_value);
    // //删除数据
    const my_ulonglong affectedRows = runCommand(cmd_str);
    return affectedRows;
}
