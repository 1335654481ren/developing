...
namespace http {
	...
	void request_handler::set_mysql(MySql *ptr_mysql){
		mysql_ = ptr_mysql;
	}
	...
	//    printf("request_path : %s\n",request_path.c_str());
	    ...
	    //处理 get 请求的参数
	    if(request_path.find("?") != std::string::npos && request_path.size() > 2)
	    {
	    //	printf("Get method parsers: == %s\n",request_path.c_str());
	    	...
	    	for(unsigned int i = 0; i < v2.size(); i++){
	    		...
	    		parser temp;
	    		SplitString(v2[i], v3,"=");
	    		//printf("%d : %s == %s\n",i,v3[0].c_str(),v3[1].c_str());
	    		temp.key = v3[0];
	    		temp.value = v3[1];
	    		parsers.numbers.push_back(temp);
	    	}

	    	rep = reply::get_post(parsers,mysql_);
			return;
	    }
