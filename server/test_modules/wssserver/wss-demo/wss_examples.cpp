...
int main() {
  ...
  // cv::Mat img_decode;
  // std::vector<uchar> data(message_str.begin(), message_str.end());
  // img_decode = cv::imdecode(data, CV_LOAD_IMAGE_COLOR);
  // imshow("aaa",img_decode);
  // waitKey(30);
  // std::vector<int> compression_params;
  // compression_params.push_back(CV_IMWRITE_JPEG_QUALITY);
  // compression_params.push_back(50);
  // imwrite("imgout.jpg", img_decode, compression_params);
    cout << "Server: Sending message :" << count++ << "\" to " << connection.get() << endl;
